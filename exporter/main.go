...
func main() {
	...
	if gethServer == "" {
		gethServer = "http://198.204.251.98:22000"
	}
...
func Routine() {
	for {
		...
		if currentBlock == nil {
			...
			previousBlockNum := currentBlock.Number().Uint64() - 1
			lastBlock,_ := eth.BlockByNumber(context.TODO(), new(big.Int).SetUint64(previousBlockNum))
			gethInfo.EffectiveBlockTime = int64(time.Unix(newBlock.Time().Int64(), 0).Sub(time.Unix(lastBlock.Time().Int64(),0)).Seconds())
...
func MetricsHttp(w http.ResponseWriter, r *http.Request) {
	...
	allOut = append(allOut, fmt.Sprintf("geth_effective_block_time %v", gethInfo.EffectiveBlockTime))
	allOut = append(allOut, fmt.Sprintf("geth_block_transactions %v", len(currentBlock.Transactions())))
	allOut = append(allOut, fmt.Sprintf("geth_block_tps %d", int64(len(currentBlock.Transactions())) / gethInfo.EffectiveBlockTime))
